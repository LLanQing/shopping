...
<script>
	...
	export default {
		...
		computed: {
			...
			//给子组件的数据
			skuImageList() {
				//如果服务器数据没有回来，skuImageList是空数组
				return this.skuInfo.skuImageList || [];
			},
		},
		//用watch监听实现表单元素修改产品个数
		/* watch: {
			skuNum(newValue, oldValue) {
				if (isNaN(newValue * 1) || newValue * 1 < 1) this.skuNum = oldValue;
				this.skuNum = parseInt(newValue);
			},
		}, */
		methods: {
			//产品的售卖属性值切换高亮
			...
			changeSkuNum() {
				...
				if (isNaN(this.skuNum * 1) || this.skuNum * 1 < 1) this.skuNum = 1;
				//正常大于1【大于1整数不能出现小数】
				this.skuNum = parseInt(this.skuNum);
			},

			//加入购物车，因为添加购物车没有任何返回数据，所以直接发送请求，不需要vuex
			async addCart() {
				// 1.await只返回成功的Promise的value，如果失败了，下面的代码不会执行，且错误提示已经在响应拦截器处理了
				await reqAddOrUpdateShopCart(this.$route.params.skuId, this.skuNum);
				//2:进行路由跳转
				/* 3:在路由跳转的时候还需要将产品的信息带给下一级的路由组件
				一些简单的数据skuNum，通过query形式给路由组件传递过去
				产品信息的数据【比较复杂:skuInfo】,通过会话存储（不持久化,会话结束数据在消失）
				本地存储|会话存储，一般存储的是字符串 */
				// 必须使用本地存储或者传递skuId，在添加购物车成功页面重新发送reqGetGoodsInfo请求获取数据
				//这里因为选择的商品的售卖属性不在skuInfo里面，所以没法传递，数据结构的问题
